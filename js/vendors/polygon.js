	 ...
	 function PolygonCreator(map){
		 ...
		 this.returnCoords = function(){
		 	return this.pen.getRawData();
		 }
	...
	// custom method to attempt to get the coordinates as an object.
...
/* NON WORKING VERSION
				var paths = this.polygon.getPlots();
				//get paths
				for(var x=0; x<paths.getAt(0).length; x++)
				{ 
				 	data.push({"lat": paths.getAt(0).j[x].lat(), "lng": paths.getAt(0).j[x].lng()})
				}

*/

// NEW VERSION wh: 2016-12-06
var paths = this.polygon.getPlots().b[0].b; // add more testing for nulls or property names. 
for(var x=0; x<paths.length; x++)
{ 
 	data.push({"lat": paths[x].lat(), "lng": paths[x].lng()})
}


				/*paths.getAt(0).forEach(function(value, index){
					//data+=(value.toString());
					data.push({"lat": value.toString()})
				});*/
				return data;
			}else {
				return null;
